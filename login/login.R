...
output$uiBanner <- renderUI({
    ...
    {
        headerPanel(
                    HTML("<p><a href='http://marxan.org' target='_blank'><img style='margin: 0px 20px' src='http://marxan.net/logos/MARXANLOGO.png' alt='Marxan logo'></a> <a href='http://www.ceed.edu.au/' target='_blank'><img src='http://marxan.net/logos/CEEDLOGO.png' alt='ARC CEED logo'></a></p> <h3>marxanio: A web app for systematic conservation planning</h3>"),
...
output$uiLogin <- renderUI({
    if (USER$Logged == FALSE)
    {
        wellPanel(
                  ...
                  br(),
                  br(),
                  a("Click here to register a new account.", href=paste0("http://",sAppServer,"/rshiny/register/"), target="_blank")
        )
    }
})
