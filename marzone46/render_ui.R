output$sidebarui <- renderUI({
    sidebarPanel(
        ...
        br(),
        a("marxanio user guide",href=paste0("http://marxan.net/downloads/",sUserGuide),target="_blank"),
        br(),
        br(),
        ...
        conditionalPanel(condition = "input.displaywhat == 'map'",
            ...
            conditionalPanel(condition = "input.displaymap == 0",
                HTML("Map rendering is slow for large polygon sets")
            ),
            conditionalPanel(condition = "input.displaymap == 1",
                conditionalPanel(condition = "input.displayleaflet == 1",
                    checkboxInput("enableleaflet","Enable Leaflet Map",value=fEnableLeaflet),
                    conditionalPanel(condition = "input.enableleafletmap == 0",
                        HTML("Leaflet is slower for large polygon sets")
                    ),
        ...
        conditionalPanel(condition="(input.displaymap == 1 & input.displaywhat == 'map' & input.map == 'runMmap') | (input.displaywhat == 'table' & input.tabletype == 'output' & input.table_o == 'mvNtable')",
        ...
        conditionalPanel(condition = "(input.displaymap == 1 & input.displaywhat == 'map' & (input.map == 'bestmap' | input.map == 'runMmap')) | input.displaywhat == 'cluster'",
            HTML(generate_ssoln_html_legend())
        ),
        conditionalPanel(condition = "input.displaymap == 1 & input.displaywhat == 'map'",
