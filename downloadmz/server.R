...
append_log_table <- function(fAll)
{
    ...
    if (!fAll)
    {
        ...
        sLine <- paste(sDate,sUserSessionKey,sFingerprint,sUserIP,sUserHostname,dQuote(sName),dQuote(sOrganisation),
                       dQuote(sCountry),sEmail,sSubscribe,sRegisterIO,sIndustry,sResearchInterest,sep=",")
        ...
        sLine <- paste(sDate,sUserSessionKey,sFingerprint,sUserIP,sUserHostname,dQuote(sName),dQuote(sOrganisation),
                       dQuote(sCountry),sEmail,sSubscribe,sRegisterIO,sIndustry,sResearchInterest,"mz",sep=",")
        write(sLine,file=sLogTable,append=TRUE)
        ...
        if (file.exists(sLogRdata))
        {
            ...
            arow <- cbind(sDate,sUserSessionKey,sFingerprint,sUserIP,sUserHostname,sName,sOrganisation,
                          sCountry,sEmail,sSubscribe,sRegisterIO,sIndustry,sResearchInterest,"mz")
            colnames(arow) <- col_names
            register_df <- rbind(register_df,arow)
...
{
    sSubject <<- paste0("Marxan with Zones download user ",sName)
    
    sMsg1 <-  paste0("Hi ",sName,"\n\n",
                     "Thankyou for downloading Marxan with Zones.\n\n")
    if (fRegisterIO)
    {
...
shinyServer(function(input, output, session, clientData) {
    ...
    system("touch /mnt/shiny/apps/revision48/downloadmz/restart.txt")
