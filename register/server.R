...
{
    col_names <- c("date","sessionkey","fingerprint","ip","userhostname","name","organisation","country","email","subscribe","industry","research interest")
    sDate <- date()

    # append csv file
    sLogTable <- paste0(sShinyPath,"/register_log.csv")
...
shinyServer(function(input, output, session, clientData) {
    ...
    system("touch /srv/shiny-server/register/restart.txt")
    ...
    output$registertable <- renderTable({
        ...
        {
            the_table <- as.data.frame(rbind(c("name",input$name),
                                         c("organisation",input$organisation),
                                         c("country",input$country),
                                         c("email",input$email),
                                         c("subscribe",input$subscribe),
                                         c("industry",input$industry),
                                         c("research interest",input$researchinterest)))
    ...
    observe({
        sSubscribe <<- "NA" # as.character(input$subscribe)
    })
    ...
    observe({
        ...
        if (input$registerAnotherUser > 0)
        {
            ...
            #updateCheckboxInput(session,"subscribe",value=TRUE)
